/**
 * generateQueryPDF.js
 * Professional PDF report for location service queries.
 * Design & structure mirrors generateLocationPDF.js — Super Tech Power Engineers Pvt. Ltd.
 */
...
function hf(doc, pageNum, totalPages, location, hindiFont, logoDataUrl) {
  // Header
  ...
  const locLine = location ? `Location: ${cleanText(location.name || "")}` : "";
  if (locLine && !logoDataUrl) T(doc, locLine, PW - MR, 10, { size: 8, color: C.white, bold: true, align: "right", hindiFont });
  if (locLine &&  logoDataUrl) T(doc, locLine, PW - MR, 24, { size: 7.5, color: C.white, bold: true, align: "right", hindiFont });
...
async function loadImageAsDataUrl(url) {
  if (!url) return null;
  try {
    const proxyUrl = url.includes("res.cloudinary.com")
      ? url.replace(/\/upload\//, "/upload/w_800,q_auto,f_jpg/")
      : url;
...
async function drawImage(doc, url, x, y, maxW, maxH) {
...
// ─── Main export ──────────────────────────────────────────────────────────────
/**
 * generateQueryPDF(location, queries)
 *
 * @param {object}   location  — location doc { name, thana, district, latitude, longitude }
 * @param {Array}    queries   — array of populated query documents
 * @returns {Promise<void>}     saves PDF to disk (browser download)
 */
export async function generateQueryPDF(location, queries = []) {
  const hindiFont = await loadHindiFont();
  const logoDataUrl = await loadLogo();

  // ── Phase 1: pre-load all images ──────────────────────────────────────────
  const allImageUrls = [];
  for (const q of queries) {
    if (q.resolution?.resolverPhoto?.url) allImageUrls.push(q.resolution.resolverPhoto.url);
    for (const p of q.resolution?.photos || []) if (p.url) allImageUrls.push(p.url);
  }
  ...
  // ── Phase 2: calculate page count ─────────────────────────────────────────
  // We'll do a dry run to count pages
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  const hf_ = (pg) => hf(doc, pg, "?", location, !!hindiFont, logoDataUrl);
  ...
  const newPage = () => {
    doc.addPage();
    pageNum++;
    hf(doc, pageNum, "?", location, hf_bool, logoDataUrl);
    return TOP;
  };
  ...
  hf(doc, 1, "?", location, hf_bool, logoDataUrl);
  let y = TOP;

  // ── Cover: Location Info ──────────────────────────────────────────────────
  y = sectionBar(doc, y, "LOCATION DETAILS", hf_bool);
  R(doc, ML, y, CW, 22, C.rowAlt);
  sd(doc, C.border); doc.setLineWidth(0.2); doc.rect(ML, y, CW, 22, "S");

  const col = CW / 3;
  kv(doc, ML + 3, y + 3,  "Location Name", cleanText(location?.name),    col, hf_bool);
  kv(doc, ML + col + 3, y + 3, "Thana", cleanText(location?.thana),       col, hf_bool);
  kv(doc, ML + col * 2 + 3, y + 3, "District", cleanText(location?.district), col, hf_bool);
  kv(doc, ML + 3, y + 12, "Report Generated", format(new Date(), "dd MMM yyyy, HH:mm"), col, hf_bool);
  kv(doc, ML + col + 3, y + 12, "Total Queries", String(queries.length), col, hf_bool);

  const statuses = ["open","in_progress","resolved","rejected"];
  const counts = {};
  statuses.forEach(s => { counts[s] = queries.filter(q => q.status === s).length; });
  kv(doc, ML + col * 2 + 3, y + 12, "Status Summary",
    `Open:${counts.open}  InProgress:${counts.in_progress}  Resolved:${counts.resolved}  Rejected:${counts.rejected}`,
    col, hf_bool, { size: 7.5 });
  ...
  if (queries.length === 0) {
    y = ensureSpace(y, 15);
    T(doc, "No queries found for this location.", ML + 3, y + 8, { size: 9, color: C.muted, bold: true, hindiFont: hf_bool });
    y += 15;
  } else {
    y = sectionBar(doc, y, `QUERY SUMMARY (${queries.length} queries)`, hf_bool);

    // Table headers
    const colWidths = [10, 30, 22, 20, 20, 28, 28, 28];
    const colLabels = ["#", "Title", "Category", "Priority", "Status", "Raised By", "Raised On", "Resolved On"];
    ...
    queries.forEach((q, idx) => {
      ...
      const vals = [
        String(idx + 1),
        cleanText(q.title).substring(0, 28),
        (q.category || "").replace("_", " "),
        q.priority || "-",
        (q.status || "").replace("_", " "),
        cleanText(q.superadmin?.name || q.superadminName || "-").substring(0, 18),
        q.createdAt ? format(new Date(q.createdAt), "dd/MM/yy") : "-",
        q.resolution?.resolvedAt ? format(new Date(q.resolution.resolvedAt), "dd/MM/yy") : "-",
      ];
  ...
  for (let qi = 0; qi < queries.length; qi++) {
    ...
    kv(doc, ML + 3, y + 14, "Raised By", cleanText(q.superadmin?.name || "-"), col2, hf_bool);
    kv(doc, ML + col2 + 3, y + 14, "Location", cleanText(q.locationName || location?.name || "-"), col2, hf_bool);
    ...
    kv(doc, ML + 3, y + 26, "Description", cleanText(q.description || "No description provided"), CW - 6, hf_bool, { size: 8 });
    ...
    if (q.status === "resolved" && q.resolution) {
      ...
      kv(doc, ML + 3, y + 18, "Resolution Details", cleanText(q.resolution.details || "-"), CW - 6, hf_bool, { size: 8 });
      y += resH + 3;

      // Resolver selfie + proof photos
      const proofPhotos = q.resolution.photos || [];
      const hasSelfie   = !!q.resolution.resolverPhoto?.url;
      const allPhotos   = [];
      if (hasSelfie) allPhotos.push({ url: q.resolution.resolverPhoto.url, label: "Resolver Photo (Selfie)" });
      proofPhotos.forEach((p, pi) => {
        if (p.url) allPhotos.push({ url: p.url, label: `Proof Photo ${pi + 1}` });
      });

      if (allPhotos.length > 0) {
        y = sectionBar(doc, y, `RESOLUTION PHOTOS (${allPhotos.length})`, hf_bool);
        const imgW = 55;
        const imgH = 45;
        const gap  = 5;
        const perRow = 3;

        for (let pi = 0; pi < allPhotos.length; pi += perRow) {
          y = ensureSpace(y, imgH + 12);
          const rowPhotos = allPhotos.slice(pi, pi + perRow);
          for (let ci = 0; ci < rowPhotos.length; ci++) {
            const px = ML + ci * (imgW + gap);
            const photo = rowPhotos[ci];
            // Caption
            R(doc, px, y, imgW, 6, C.sectionBg);
            T(doc, photo.label, px + 2, y + 4.5, { size: 7, color: C.accent, bold: false, hindiFont: hf_bool });
            // Image
            const cached = imageCache.get(photo.url);
            if (cached) {
              try {
                const img = new Image();
                img.src = cached;
                const ar = img.naturalWidth / img.naturalHeight || 1;
                let drawW = imgW, drawH = drawW / ar;
                if (drawH > imgH) { drawH = imgH; drawW = drawH * ar; }
                doc.addImage(cached, "JPEG", px, y + 6, drawW, drawH, undefined, "FAST");
              } catch (_) {
                T(doc, "[Image unavailable]", px + 2, y + imgH / 2, { size: 7, color: C.muted, hindiFont: hf_bool });
              }
            } else {
              R(doc, px, y + 6, imgW, imgH, C.rowAlt);
              T(doc, "[Image unavailable]", px + 2, y + imgH / 2 + 6, { size: 7, color: C.muted, hindiFont: hf_bool });
            }
          }
          y += imgH + 8;
        }
    ...
    if (q.timeline?.length > 0) {
      ...
      for (const ev of q.timeline) {
        // Calculate row height based on remark length
        ...
        if (remarkText) {
          // Re-split with correct font size set
          doc.setFontSize(7);
  ...
  for (let p = 1; p <= totalPages; p++) {
    doc.setPage(p);
    hf(doc, p, totalPages, location, hf_bool, logoDataUrl);
  }

  // ── Save ──────────────────────────────────────────────────────────────────
  const locationSlug = (location?.name || "location").replace(/\s+/g, "_").toLowerCase();
  const dateStr = format(new Date(), "yyyyMMdd_HHmm");
  doc.save(`SQR_${locationSlug}_${dateStr}.pdf`);
}

export default generateQueryPDF;